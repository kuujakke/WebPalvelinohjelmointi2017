...
describe "BeermappingApi" do
  describe "in case of cache miss" do
    ...
    it "When HTTP GET returns empty, empty list is returned" do

      canned_answer = <<-END_OF_STRING
        <?xml version='1.0' encoding='utf-8' ?><bmp_locations><location><id></id><name></name><status></status><reviewlink></reviewlink><proxylink></proxylink><blogmap></blogmap><street></street><city></city><state></state><zip></zip><country></country><phone></phone><overall></overall><imagecount></imagecount></location></bmp_locations>
      END_OF_STRING

      stub_request(:get, /.*nummela/).to_return(body: canned_answer, headers: { 'Content-Type' => "text/xml" })

      places = BeermappingApi.places_in("nummela")

      expect(places.size).to eq(0)
    end
    it "When HTTP GET returns many, list containing many places is returned" do
      canned_answer = <<-END_OF_STRING
      ...
      expect(places.second.name).to eq("Pyynikin kà¤sityà¶là¤ispanimo")
